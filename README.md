...
6. **自动清理运行记录**：每次运行后只保留最近 6 次 Actions 运行，其余自动删除。
7. **北京时间**：日志与推送消息中的时间均采用北京时间（UTC+8），精确到秒。
...
- 调度：`5,35 * * * *`（UTC），每小时运行两次，避开整点高峰提升准时率。
- 每次运行先读取 `log.txt`，若其中记录了「今天已经签到成功」则直接退出（退出码 0，不修改 `log.txt`）。
- 否则执行登录 + 签到，并把结果写入 `log.txt`。
- 签到成功（退出码 0）时，向 Telegram 推送一条美化后的消息，并将更新后的 `log.txt` 提交回仓库；
  签到失败（退出码 1）时不做提交、不推送，下次运行会重试。
- 每次运行结束都会清理旧的运行记录（`Mattraks/delete-workflow-runs`），只保留最近 6 次。
